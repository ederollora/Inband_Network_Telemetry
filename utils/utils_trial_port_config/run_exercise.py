...
class ExerciseRunner:
    ...
    RUNTIME_RULES_DIRECTORY = "../runtime_rules/"
    ...
    def __init__(self, topo_file, log_dir, pcap_dir,
        ...
        self.mon_server = topo['mon_server']
        self.veths = topo['veths']
        #print(self.mon_server)
    ...
    def run_exercise(self):
        ...
        subprocess.Popen(['bash ../utils/unconfig_intfs.sh'], shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        self.net.stop()
    ...
    def create_network(self):
        ...
        for switch_name in self.veths.iterkeys():
            switch = self.net.switches[int(switch_name[1:2])-1]
            root = Node( 'root', inNamespace=False )
            intf = self.net.addLink( root, switch ).intf1
            ip='10.0.223.2/24'
            root.setIP(ip, intf=intf)
            subprocess.Popen(['bash ../utils/config_intf.sh'], shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)

        #switch = self.net.hosts[3]
        #root = Node( 'root', inNamespace=False )
        #intf = self.net.addLink( root, switch ).intf1
        #ip='10.0.223.2/24'
        #root.setIP(ip, intf=intf)
        #p = switch.newPort()
        #sw.addIntf("mon-gate", port=p)
    ...
    def program_switch_p4runtime(self, sw_name, sw_dict):
        ...
        runtime_json = self.RUNTIME_RULES_DIRECTORY+sw_dict['runtime_json']
    ...
    def program_switch_cli(self, sw_name, sw_dict):
        ...
        cli_input_commands = self.RUNTIME_RULES_DIRECTORY+sw_dict['cli_input']
    ...
    def program_hosts(self):
        ...
        for host_name in self.topo.hosts():
            ...
            #if(host_name == self.mon_server["host"]):
                #h.cmd('ip link set %s address %s' % (self.mon_server["db_gw_intf"], self.mon_server["eth1_intf_mac"]))
                #h.cmd('ifconfig %s %s netmask 255.255.255.0 broadcast %s.255' % (self.mon_server["db_gw_intf"], self.mon_server["eth1_intf_ip"], self.mon_server["eth1_intf_ip"].rsplit('.', 1)[0]))
                #h.cmd('arp -i %s -s %s %s' % (self.mon_server["db_gw_intf"], self.mon_server["bridge_ip"], self.mon_server["bridge_mac"]))
                #h.cmd('route add -net %s.0 netmask 255.255.255.0 dev %s' % (self.mon_server["db_ip"].rsplit('.', 1)[0], self.mon_server["db_gw_intf"]))
                #subprocess.Popen(['bash ../utils/config_intfs.sh'], shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
